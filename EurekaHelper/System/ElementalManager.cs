...
namespace EurekaHelper.System
{
    public class ElementalManager : IDisposable
    {
        ...
        private void OnUpdate(IFramework framework)
        {
            ...
            {
                if (Elementals.Exists(x => x.ObjectId == elemental.GameObjectId))
                {
                    var match = Elementals.FirstOrDefault(x => x.ObjectId == elemental.GameObjectId);
                    if (match == null)
                        continue;
                ...
                var eurekaElemental = new EurekaElemental(elemental.Name.TextValue, DalamudApi.ClientState.TerritoryType, elemental.Position, (uint)elemental.GameObjectId);
